...
public class MeFragment extends BaseFragment {
    ...
    //    com.netease.nimlib.g.l @ 0x1302a760
    ...
    public void initData() {
        if (null == userInforequestCallback) {
            return;
        }
    ...
    public void onDestroy() {
        super.onDestroy();
        userInforequestCallback = null;
        if (mQRCodeCardDialog != null)
            mQRCodeCardDialog.dismiss();
        NimUserInfoSDK.getUserInfosFormServer(accountList, null);
    }

    List<String> accountList = new ArrayList<>();
    RequestCallback userInforequestCallback;

    private void getUserInfoFromRemote() {
        accountList.clear();
        accountList.add(UserCache.getAccount());
        userInforequestCallback = new RequestCallback<List<NimUserInfo>>() {
            @Override
            public void onSuccess(List<NimUserInfo> param) {
                if (param.size() > 0) {
                    mNimUserInfo = param.get(0);
                    initData();
                }
        ...
        NimUserInfoSDK.getUserInfosFormServer(accountList, userInforequestCallback);
    }
