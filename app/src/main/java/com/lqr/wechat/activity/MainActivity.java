...
//    com.netease.nimlib.g.e @ 0x12db6720
    ...
    protected void onDestroy() {
        ...
        try {
            Field mNextServedViewField = InputMethodManager.class.getDeclaredField("mNextServedView"); //获取私有和共有属性
            ...
            Object mNextServedView = mNextServedViewField.get(im);
            Object mServedView = mServedViewField.get(im);
            Object mCurRootView = mCurRootViewField.get(im);

            LogUtils.e("========mNextServedView="+mNextServedView);


            if (null != mNextServedView) {
                Context context = ((View) mNextServedView).getContext();
                LogUtils.e("========context="+context+"=======this="+this);
                if (null != context && context == this) {
                    mNextServedViewField.set(im,null);
                }
            }
        ...
        NimUserInfoSDK.observeUserInfoUpdate(mUserInfoObserver, false);
        NimFriendSDK.observeFriendChangedNotify(mFriendChangedModifyObserve, false);
        NimSystemSDK.observeReceiveSystemMsg(mSystemMessageObserver, false);
        NimTeamSDK.observeTeamRemove(mMessageObserver, false);
...
//    }
    ...
    Observer mUserInfoObserver;

    /**
     * 监听用户信息更新
     */
    private void observeUserInfoUpdate() {
        mUserInfoObserver = new Observer<List<NimUserInfo>>() {
            @Override
            public void onEvent(List<NimUserInfo> nimUserInfos) {
                mMeFragment.initData();
            }
        };
        NimUserInfoSDK.observeUserInfoUpdate(mUserInfoObserver, true);
    }

    Observer mFriendChangedModifyObserve;

    /**
     * 监听好友关系的变化
     */
    private void observeFriendChangedNotify() {
        mFriendChangedModifyObserve = new Observer<FriendChangedNotify>() {
            @Override
...
//                List<String> deletedFriendAccounts = friendChangedNotify.getDeletedFriends(); // 删除好友或者被解除好友
        ...
        NimFriendSDK.observeFriendChangedNotify(mFriendChangedModifyObserve, true);
    }

    Observer mMessageObserver;

    /**
     * 监听群聊关系的变化
     */
    private void observeTeamChangedNotify() {
        mMessageObserver = new Observer<Team>() {
            @Override
            public void onEvent(Team team) {
                mMessageFragment.initData();
            }
        };
        NimTeamSDK.observeTeamRemove(mMessageObserver, true);
...
//        }, true);
    ...
    Observer mSystemMessageObserver;

    /**
     * 监听系统消息通知
     */
    private void observeReceiveSystemMsg() {
        mSystemMessageObserver = new Observer<SystemMessage>() {
            @Override
        ...
        NimSystemSDK.observeReceiveSystemMsg(mSystemMessageObserver, true);
    }
}
